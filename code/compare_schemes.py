...
def plot_cost_vs_accuracy(results, fp32_own_error, outdir):
    ...
    ax.set_title("Cost against accuracy, every scheme on the same cropped Marmousi")
...
def plot_block_size(block_results, outdir):
    ...
    ax.set_ylabel("relative error of the shot record")
    ax.set_title(f"Effect of block size at a fixed {BLOCK_SWEEP_BITS} mantissa bits")
    ax.grid(alpha=0.3, which="both")
    fig.tight_layout()
